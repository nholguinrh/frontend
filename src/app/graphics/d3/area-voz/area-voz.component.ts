...
@Component({
  selector: 'app-area-voz',
  templateUrl: './area-voz.component.html'
})
export class AreaVozComponent implements OnInit {
  private static id = 0;
  ...
  ngOnInit(): void {
    this.componentId = ++AreaVozComponent.id;
  }
  ...
  private initDraw() {
    ...
    const mouseMove = (ev: any) => {
      ...
      if (this._scale === 'week') {
        ...
        this.formatShort = d3.timeFormat('%d - %H:%Mhrs');
      ...
      if(d.value != null){
        ...
        toltp
          .select('text')
          .html((_) => {
          return `<span style='font-size: 9px;'>${this.formatShort(
              d.date
  ...
  defineXAxisTicks() {
    ...
    if (this._scale === 'week') {
      ...
      this.format = d3.timeFormat('%H:%Mhrs');
    } else if (this._scale === 'dayMonth') {
      this.format = d3.timeFormat('%d- %m');
    } else {
    ...
    this.xAxis = d3
      ...
      if (this._scale === 'week') {
        ...
        //this.xAxis.tickValues(this.xScale.ticks(this._data.length + 1));
  ...
  createAxis() {
    ...
    this.svg.selectAll('text').attr('transform', (g) => {
      if (this._scale == 'minute') {
        return 'translate(0,5) rotate(0)';
      } else {
        return 'translate(0,25) rotate(0)';
      }
    });
    }
